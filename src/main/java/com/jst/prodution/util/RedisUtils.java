...
public class RedisUtils {
	...
	private static final String WORKPATH = Thread.currentThread().getContextClassLoader().getResource("").getPath();
	private static final String REDISPATH = WORKPATH + "config" + File.separator + "RedisConfig.xml";//Redis配置文件路径
	private static ILogger log = new ILogger(ApiConstants.JST_FINANCE_API, RedisUtils.class);
	...
	private static void initialPool() {
		...
		JDomHandler jdom = new JDomHandler();
		try {
			jdom.loadXmlByPath(REDISPATH);
		} catch (Exception e) {
			log.error("读取redis配置文件失败,可能原因:RedisConfig.xml不存在");
		}
		String ip = jdom.getNodeValue("config/Ip");
		String port = jdom.getNodeValue("config/Port");
		String maxTotal = jdom.getNodeValue("config/MaxTotal");
		String maxIdle = jdom.getNodeValue("config/MaxIdle");
		String maxWaitMillis = jdom.getNodeValue("config/MaxWaitMillis");
		String testOnBorrow = jdom.getNodeValue("config/TestOnBorrow");
		if(StringUtils.isBlank(ip)){
			throw new JstException("API0001", "01", "RedisConfig.xml未配置服务器IP");
		}
		if(StringUtils.isBlank(port)){
			throw new JstException("API0001", "01", "RedisConfig.xml未配置服务器Port");
		}
		...
		if(StringUtils.isBlank(maxWaitMillis)){
			maxWaitMillis = "-1";
		}
		if(StringUtils.isBlank(testOnBorrow)){
			testOnBorrow = "false";
		}
		log.info("Redis配置参数【maxTotal:[" + maxTotal + "]maxIdle:[" + maxIdle + "]maxWaitMillis:[" + maxWaitMillis + "]testOnBorrow:[" + testOnBorrow + "]】");
	...
	private static void initialShardedPool() {
		...
		shards.add(new JedisShardInfo("10.101.130.212", 6379, "localhost"));
	 ...
	 * @des set对象
	 * @param1 String key
	 * @param2 Object 可以是String,Object,List,MAP
	 * @return
	 */
	 ...
	 * @des 获取对象
	 * @param1 String key
	 * @return Object 可以是String,Object,List,MAP
	 */

	public static Object getObject(String key) {
		byte[] byteArr = jedis.get(key.getBytes());
		Object Object = SerializeUtil.unserialize(byteArr);
		// closeResource();
		return Object;
	}
	...
	public static Long deleteObject(String key) {
		Long retval = jedis.del(key.getBytes());
		// closeResource();
		return retval;
	}
	...
	public static void main(String[] args) {
		...
		inMap.put("1", "11111");
		inMap.put("2", "22222");
		RedisUtils.setObject("1", inMap);
		
		//从Redis缓存获取数据
		Map<String, Object> outMap = (Map<String, Object>) RedisUtils.getObject("1");
		System.out.println(outMap.get("1").toString());
